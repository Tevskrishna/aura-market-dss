...
def threat_gauge(score: int, verdict: str, color: str) -> None:
    """Threat gauge — calm enterprise instrument."""
    ...
    st.html(
        ...
        <div class="copilot-gauge-wrap">
          <svg viewBox="0 0 240 140" class="copilot-gauge" role="img" aria-label="Launch threat score {score}">
            ...
            <text x="120" y="116" text-anchor="middle" fill="{html.escape(color)}" font-size="13" font-weight="800" letter-spacing="1.2" font-family="Exo 2,sans-serif">{html.escape(verdict)}</text>
          </svg>
          <div class="copilot-gauge-caption">Threat score · 0 clear → 100 abort</div>
        </div>
        """
    )
